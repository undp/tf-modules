  ...
  asg_list = flatten([
    for nsg_key, asg_list in local.clean_asg_set : [
      for asg_name in asg_list : {
        ...
        # TODO:20200103:002
        #   Consider crating ASGs in a separate module.

        key      = "${nsg_key}_${asg_name}"
        name     = "${asg_name}_${nsg_key}"
...
resource "azurerm_application_security_group" "nsg_rules" {
...
resource "azurerm_network_security_rule" "nsg_rules" {
  ...
  source_application_security_group_ids      = lookup(each.value, "source_asg", null) == null ? [] : [azurerm_application_security_group.nsg_rules["${lower(each.value.nsg_key)}_${each.value.source_asg}"].id]
  destination_application_security_group_ids = lookup(each.value, "destination_asg", null) == null ? [] : [azurerm_application_security_group.nsg_rules["${lower(each.value.nsg_key)}_${each.value.destination_asg}"].id]
